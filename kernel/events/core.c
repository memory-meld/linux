...
void perf_sched_cb_inc(struct pmu *pmu)
{
	...
	// pr_debug_ratelimited("%s: enabling __perf_event_task_sched_out() => perf_pmu_sched_task()\n", __func__);
...
int kernel_perf_event_open(struct perf_event_attr *attr_ptr, pid_t pid, int cpu,
...
EXPORT_SYMBOL_GPL(kernel_perf_event_open);
...
SYSCALL_DEFINE5(perf_event_open,
	...
	if (err)
		return err;
	return kernel_perf_event_open(&attr, pid, cpu, group_fd, flags);
}
