...
fn main() -> Result<()> {
    ...
    let nelems = args.len / args.threads / args.granularity;
    let dist = ZipfDistribution::new(nelems, args.zipf).unwrap();
    ...
    fn next<R: Rng + ?Sized>(&self, rng: &mut R) -> usize {
        ...
        loop {
                    ...
                    - ZipfDistribution::h(k64, self.exponent)
            {
                ...
                // the original implementation returns k directly
                // let mut s = DefaultHasher::new();
                // k.hash(&mut s);
                // return s.finish() as usize % self.num_elements as usize;
                // return self.num_elements as usize - k;
                return k - 1;
            }
        }
    }
